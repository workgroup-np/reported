    ...
    while($query_ajax->have_posts()): $query_ajax->the_post();
        ...
        if ( ! empty( $categories ) ) {
            foreach( $categories as $category ) {
                $output .= '<a class="tbeer-category-meta" href="' . esc_url( get_category_link( $category->term_id ) ) . '" alt="' . esc_attr( sprintf( __( 'View all posts in %s', 'reported' ), $category->name ) ) . '">' . esc_html( $category->name ) . '</a>' . $separator;
            }
        }
        if($image){
...
if($_POST['action_type'] == 'loadmore_trending' ){    
    $posts_per_page=1; $h='310';$w='310';
    $args_ajax = array(
        'posts_per_page'   => $posts_per_page,
        'orderby'          => 'date',
        'order'            => 'DESC',
    ...
    while($query_ajax->have_posts()): $query_ajax->the_post();
        ...
        $html=' <div class="tbeer-popular-news-post"><div class="tbeer-popular-news-img"><img src="'.esc_url($url).'" alt="'.esc_attr($text).'"></div><div class="tbeer-popular-news-details"><h3 class="tbeer-news-post-heading"><a href="'.get_the_permalink().'">'.get_the_title().'</a></h3></div></div>';
    endwhile;
    echo $html;
    wp_reset_postdata();
}
